...
public class Recommendation
{
    ...
    //Mapping Recommendation To User(many to one) i.e many recommendation to one user
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "user_id", nullable = false, foreignKey = @ForeignKey(name="fk_recommendation_user"))
    @JsonIgnore
    private User user;

    //Mapping Recommendation To Activity(many to one) i.e many recommendation to one activity
    @ManyToOne(fetch = FetchType.LAZY)
